...
<?php if($scrollView){ ?>
    <div class="overflow-x-auto scroll-container lg:overflow-x-hidden">
        <div class="grid grid-flow-col auto-cols-max gap-3.75 pb-5">
            <?php get_template_part('template-parts/product-card-rating', null, array('cardWidth' => $cardWidth)); ?>
        </div>
    </div>
    ...
    <section class="splide product-rating-card-splide" aria-label="Basic Structure Example">
        <div class="splide__track">
            <ul class="splide__list">
                <?php get_template_part('template-parts/product-card-rating', null, array('carousel' => $carousel, 'cardWidth' => $cardWidth)); ?>
            </ul>
        </div>
    </section>
<?php } else { ?>
