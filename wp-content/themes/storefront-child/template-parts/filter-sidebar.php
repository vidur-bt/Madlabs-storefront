...
<aside>
    ...
    <div>
        <?php if(wp_is_mobile()){ ?>
            <button command="show-modal" commandfor="drawer" class="bg-gray-2 w-[61px] h-[23px] rounded-full flex items-center justify-center gap-1 text-xs text-dark-blue">
                <img src="<?php echo get_stylesheet_directory_uri(); ?>/assets/icons/filter-icon.svg ?>" alt="">
                <span>Filter</span>
            </button>
            <el-dialog>
                        ...
                        <el-dialog-panel class="group/dialog-panel relative ml-auto block size-full max-w-md transform transition duration-500 ease-in-out data-closed:translate-x-full sm:duration-700">
                            ...
                            <div class="relative flex h-full flex-col overflow-y-auto bg-white py-6 shadow-xl">
                                <div class="px-4 sm:px-6">
                                    <h2 id="drawer-title" class="text-2xl font-semibold text-gray-900">Filter</h2>
                                </div>
        ...
        <?php } else { 
            get_template_part('template-parts/filter-cards', null, array('page' => $page)); 
        } ?>
